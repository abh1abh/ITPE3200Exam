...
export const fetchAllWorkers = async () => { // Fetch all healthcare workers
...
export const fetchWorker = async (id: number) => { // Fetch healthcare worker by ID
...
export const fetchWorkerBySelf = async () => { // Fetch healthcare worker data for the authenticated user
...
export const updateWorker = async (id: number, worker: UpdateUserDto) => { // Update healthcare worker by ID
...
export const deleteWorker = async (id: number) => { // Delete healthcare worker by ID
  const response = await fetch(`${API_URL}/api/HealthcareWorker/${id}`, {
    method: "DELETE",
    headers: getAuthHeaders(),
  });
