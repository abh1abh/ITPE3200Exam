...
export const fetchAllClients = async () => { // Fetch all clients
...
export const fetchClient = async (id: number) => { // Fetch client by ID
...
export const updateClient = async (id: number, client: UpdateUserDto) => { // Update client by ID
...
export const fetchClientBySelf = async () => {
  const response = await fetch(`${API_URL}/api/Client/clientauth`, { // Fetch client data for the authenticated user
    method: "GET",
    headers: getAuthHeaders(),
  });
  return handleResponse(response); // Return client data
};
export const deleteClient = async (id: number) => { // Delete client by ID
  const response = await fetch(`${API_URL}/api/client/${id}`, {
    method: "DELETE",
    headers: getAuthHeaders(),
  });
