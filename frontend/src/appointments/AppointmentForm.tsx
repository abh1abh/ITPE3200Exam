...
interface AppointmentFormProps {onAppointmentChanged: (newAppointment: Appointment) =>void;initialData?: Appointment;isUpdate?: boolean;}
const AppointmentForm: React.FC<AppointmentFormProps>= ({onAppointmentChanged,initialData,isUpdate = false,}) => {
...
useEffect(() => {
    ...
    try {
        ...
        fetch(`${import.meta.env.VITE_API_URL}/api/AvailableSlot`,{headers: { Authorization: `Bearer ${token}` },}),
        fetch(`${import.meta.env.VITE_API_URL}/api/HealthcareWorker`,{headers: { Authorization: `Bearer ${token}` },}),
        fetch(`${import.meta.env.VITE_API_URL}/api/Client`, {headers: { Authorization: `Bearer ${token}` },}),]);
        if (!slotr.ok|| workerr.ok ||!clienr.ok)
        throw new Error("failed");
        const [slotData, workerData, clientData] = await Promise.all([
        slotr.json(),workerr.json(),
        clienr.json(),
        ]);
...
const handleSlotChange = (slotId: number) =>{setAvailableSlotId(slotId);
...
return (
    ...
    <Form.Group controlId="formClientId" className="mb-3">
        ...
        <Form.Select value={clientId}
        onChange={(e)=> setClientId(Number(e.target.value))}required>
        <option value="">Select a client</option>
        {clients.map((cl)=>(<option key={cl.clientId} value={cl.clientId}>{cl.name||`Client ${cl.clientId}`}
            </option>))}
        </Form.Select>
    </Form.Group>
    <Form.Group controlId="formWorkerId" className="mb-3">
        <Form.Label>Healthcare Worker</Form.Label>
        <Form.Select value={healthcareWorkerId}
        onChange={(e) =>setHealthcareWorkerId(Number(e.target.value))}required>
    ...
    <Form.Group controlId="formAvailableSlotId" className="mb-3">
        ...
        {availableSlots.length > 0 ? (
            ...
            </option>))):(<option disabled>No available slots</option>)}
        </Form.Select>
    </Form.Group>
