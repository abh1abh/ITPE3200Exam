...
public static class DBInit
{
    ...
    public static async Task SeedAsync(IServiceProvider sp, SeedResult authSeed) // Seed method to initialize the database
    {
        ...
        appDbContext.ChangeLogs.AddRange(
            new ChangeLog { AppointmentId = appts[0].Id, ChangeDate = DateTime.Now, ChangedByUserId = workers[0].AuthUserId, ChangeDescription = "Rescheduled due to patient request" }
        );
        await appDbContext.SaveChangesAsync();
